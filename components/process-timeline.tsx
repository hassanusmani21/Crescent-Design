...
function ProcessTimelineNode({ active }: { active: boolean }) {
  return (
    <span className={`process-journey__node-wrap${active ? " is-active" : ""}`} aria-hidden="true">
      <span className="process-journey__node" />
    </span>
  );
}
...
function ProcessTimelineSegment({ active }: { active: boolean }) {
  return (
    <span className={`process-journey__segment${active ? " is-active" : ""}`} aria-hidden="true">
      <span className="process-journey__segment-fill" />
    </span>
  );
}
...
export function ProcessTimeline({ stages }: { stages: ProcessStage[] }) {
  ...
  return (
    <ol className="process-journey" aria-label="Process timeline" onMouseLeave={() => setActiveIndex(0)}>
      {stages.map((stage, index) => {
        ...
        return (
          <li key={stage.number} className={`process-journey__step${isActive ? " is-active" : ""}`}>
            {hasSegment ? <ProcessTimelineSegment active={isSegmentActive} /> : null}
            <button
              type="button"
              className="process-journey__button"
              aria-pressed={isActive}
              onClick={() => setActiveIndex(index)}
              onFocus={() => setActiveIndex(index)}
              onMouseEnter={() => setActiveIndex(index)}
            >
              <ProcessTimelineNode active={isActive} />
              <span className="process-journey__number">{stage.number}</span>
              <span className="process-journey__icon">
                <ProcessIcon title={stage.title} />
              </span>
              <span className="process-journey__title">{stage.title}</span>
              <span className="process-journey__accent" aria-hidden="true" />
              <span className="process-journey__description">{stage.description}</span>
            </button>
          </li>
        );
      })}
